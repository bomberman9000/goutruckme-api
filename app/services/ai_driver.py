...
class AIDriver:
    ...
    def calculate_route(self, from_city: str, to_city: str) -> dict:
        """Расчёт маршрута (заглушка для API карт)."""
        
        # TODO: Интеграция с Яндекс.Карты / 2GIS / OpenStreetMap
        
        # Примерные данные
        routes = {
            ("москва", "санкт-петербург"): {"distance": 710, "duration": 8, "tolls": 1500},
            ("москва", "казань"): {"distance": 820, "duration": 10, "tolls": 800},
            ("москва", "нижний новгород"): {"distance": 420, "duration": 5, "tolls": 400},
            ("москва", "воронеж"): {"distance": 520, "duration": 6, "tolls": 300},
            ("москва", "ростов-на-дону"): {"distance": 1070, "duration": 12, "tolls": 2000},
        }
        
        key = (from_city.lower(), to_city.lower())
        route_data = routes.get(key, {"distance": 500, "duration": 6, "tolls": 500})
    ...
    def _get_route_warnings(self, route_data: dict) -> list:
        ...
        if route_data["tolls"] > 1000:
            warnings.append({
                "type": "toll",
                "message": f"⚠️ Платные дороги: {route_data['tolls']}₽"
            })
        
        if route_data["duration"] > 8:
            warnings.append({
                "type": "rest",
