...
# Маршруты для мониторинга ATI цен
ATI_MONITORED_ROUTES = [
    ("Самара", "Казань"),
    ("Москва", "Казань"),
    ("Самара", "Москва"),
    ("Москва", "Екатеринбург"),
...
ATI_PRICES_LOG = "ati_prices_log.json"


async def ati_price_check_job():
    """Check ATI.SU market rates for monitored routes every 4 hours."""
    import asyncio
    import json
    import os
    import random
    ...
    try:
        from src.parser_bot.ati_analyzer import get_route_rate
    except Exception as e:
        logger.error("ati_price_check: import error: %s", e)
        return

    results = []
    for from_city, to_city in ATI_MONITORED_ROUTES:
        try:
            ...
            if rate:
                results.append({
                    "from": from_city,
                    "to": to_city,
                    "price_rub": rate.price_rub,
                    "distance_km": rate.distance_km,
                    "price_per_km": rate.price_per_km,
                    "loads_count": rate.loads_count,
                    "min_price": min(rate.raw_prices) if rate.raw_prices else None,
                    "max_price": max(rate.raw_prices) if rate.raw_prices else None,
                    "checked_at": datetime.utcnow().isoformat(),
                })
                logger.info(
                    "ATI price check: %s→%s %s ₽ (n=%d)",
                    from_city, to_city, rate.price_rub, rate.loads_count,
                )
            else:
                logger.info("ATI price check: %s→%s no data", from_city, to_city)
        except Exception as e:
            logger.warning("ATI price check error %s→%s: %s", from_city, to_city, e)

        await asyncio.sleep(random.uniform(2, 4))

    if not results:
        logger.warning("ati_price_check: no results obtained")
        return

    # Сохраняем в лог-файл
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", ATI_PRICES_LOG)
    try:
        existing = []
        ...
        # Держим только последние 500 записей
        if len(existing) > 500:
            existing = existing[-500:]
        with open(log_path, "w", encoding="utf-8") as f:
            json.dump(existing, f, ensure_ascii=False, indent=2)
        logger.info("ATI prices saved to %s (%d records)", log_path, len(existing))
    except Exception as e:
        logger.error("ATI price log write error: %s", e)

    # Отправляем краткий отчёт администратору
    try:
        from src.core.config import settings
        from src.bot.bot import bot

        if settings.admin_id:
            lines = ["📊 <b>Ставки ATI.SU</b> (обновление каждые 4ч)\n"]
            for r in results:
                km = f" {r['distance_km']} км" if r["distance_km"] else ""
                per_km = f" ({r['price_per_km']} ₽/км)" if r["price_per_km"] else ""
                price = f"{r['price_rub']:,}".replace(",", "\u202f")
                rng = f"[{r['min_price']:,}–{r['max_price']:,} ₽, {r['loads_count']} гр.]" if r["min_price"] else ""
                lines.append(f"✅ {r['from']} → {r['to']}:{km} <b>{price} ₽</b>{per_km} {rng}")
            await bot.send_message(settings.admin_id, "\n".join(lines), parse_mode="HTML")
    except Exception as e:
        logger.debug("ATI price check: admin notify skipped: %s", e)
...
def setup_scheduler():
    ...
    scheduler.add_job(ati_price_check_job, IntervalTrigger(hours=4), id="ati_price_check")
    scheduler.start()
