...
from app.services.bot_webhooks import send_sync_to_bot_sync
...
def _vehicle_to_dict(vehicle: Vehicle, db: Session, *, matching_loads: Optional[int] = None) -> dict:
    ...
    return {
        ...
        "is_priority": bool(getattr(vehicle, "is_priority", False)),
    }
...
def _search_market_vehicles(db: Session, parsed: dict[str, Any], *, limit: int, recent_only: bool) -> list[dict[str, Any]]:
    ...
    if recent_only and not any(parsed.get(key) for key in ("query", "from_city", "to_city", "body_type", "vehicle_kind", "weight_tons")):
        items = (
            query.order_by(Vehicle.is_priority.desc(), Vehicle.created_at.desc(), Vehicle.id.desc())
            .limit(limit)
            .all()
        )
        return [_vehicle_market_teaser(vehicle, db) for vehicle in items]

    candidates = query.order_by(Vehicle.is_priority.desc(), Vehicle.created_at.desc(), Vehicle.id.desc()).limit(250).all()
...
def list_vehicles(
    ...
    vehicles = query.order_by(Vehicle.is_priority.desc(), Vehicle.created_at.desc(), Vehicle.id.desc()).all()
...
def create_vehicle(
    ...
    vehicle = Vehicle(
        ...
        is_priority=_is_pro_user(owner),
    ...
    _sync_vehicle_to_tg_bot(vehicle, owner)
