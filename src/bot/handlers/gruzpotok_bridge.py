...
async def sync_vehicle_to_site(vehicle: Any, *, user_id: int) -> bool:
    """Sync a UserVehicle from the bot to gruzpotok-api vehicles table."""
    ...
    try:
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            ...
            if resp.status_code == 200:
                logger.info("bridge.sync_vehicle ok vehicle_id=%s user=%s", vehicle.id, user_id)
                return True
            logger.warning("bridge.sync_vehicle status=%s", resp.status_code)
            return False
    except Exception as exc:
        logger.warning("bridge.sync_vehicle failed: %s", exc)
        return False
