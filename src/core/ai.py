...
async def parse_cargo_nlp(text: str) -> dict | None:
    ...
    prompt = (
        ...
        "ПРАВИЛА:\n"
        "1. Внимательно смотри на предлоги: 'из', 'от' -> from_city; 'в', 'до', 'на' -> to_city.\n"
        "2. Если города указаны просто через тире (А-Б), то первый - from, второй - to.\n"
        "3. Если есть слово 'наоборот', поменяй города местами.\n"
        "4. Вес всегда в Тоннах (число).\n\n"
        f"Текст: \"{raw_text}\"\n\n"
        "Верни ТОЛЬКО JSON:\n"
        "{\n"
        ...
        '  "price": int или null,\n'
        '  "cargo_type": "ТНП/Металл/Продукты/...",\n'
