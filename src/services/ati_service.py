...
Экспорт кук: python scripts/ati_refresh_cookies.py

Запуск теста:
    python -m src.services.ati_service
"""
import asyncio
import json
import logging
...
def _load_cookies() -> dict[str, str]:
    ...
    if not os.path.exists(STATE_FILE):
        raise FileNotFoundError(
            ...
            "Запусти: python scripts/ati_refresh_cookies.py"
        )
...
async def get_route_rate(
    ...
    try:
        ...
        logger.error("ATI cookie error: %s", e)
        return None
    ...
    async with httpx.AsyncClient(
        ...
        if not data.get("isUserAuthorized"):
            logger.error("ATI session expired. Run: python scripts/ati_refresh_cookies.py")
            return None
