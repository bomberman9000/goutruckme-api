...
class PriceRequest(BaseModel):
    ...
    vehicle: str = Field(default="тент", max_length=50)
    user_id: int | None = None


@router.post("/ai/price")
async def ai_price(request: PriceRequest):
    """AI-оценка ставки маршрута: рыночные данные + Kimi-анализ."""
    await _check_limit(request.user_id)
    ...
    try:
        ...
        market_data = await predict_route_price(request.from_city, request.to_city)
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning("ai_price.market_data error=%s", e)

    try:
        from src.core.services.geo_service import get_geo_service
        geo = await get_geo_service().resolve_route(request.from_city, request.to_city)
        if geo:
            distance_km = geo.distance_km
    except Exception:
        pass

    result = await kimi_service.price_mode(
    ...
    if request.user_id:
        await log_ai_request(
            request.user_id,
            "price",
            f"{request.from_city}→{request.to_city} {request.weight}т",
            result,
        )
    return result
