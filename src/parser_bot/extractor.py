...
_CARGO_STRONG_RE = re.compile(
    r"(?:"
    r"\d+\s*(?:т(?:онн|онны|онна)?|t(?:on(?:na|a)?)?)\b"
    r"|[→➞➡]"
    r"|>{2,}"
    r"|\b(?:тент|tent|реф(?:рижератор)?|ref|трал|борт|контейнер|фура|fura|газель|изотерм|шаланда)\b"
    r"|\b(?:ставка|фрахт|погрузка|выгрузка|загрузка)\b"
    r"|\b(?:yuk\s+bor|mashina\s+kerak)\b"
    r")",
