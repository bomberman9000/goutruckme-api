...
_MODES = {
    ...
    "docs":      ("📄 Документ", "Опиши что нужно: договор, заявка или сообщение клиенту"),
...
# ---------------------------------------------------------------------------
# /ai — entry point (command + callback from main menu)
# ---------------------------------------------------------------------------
...
async def _show_ai_menu(target, state: FSMContext) -> None:
    """Show AI mode selection. target is Message or CallbackQuery."""
    await state.set_state(AIAssistant.choose_mode)
    text = "🤖 <b>AI-ассистент ГрузПоток</b>\n\nВыбери режим:"
    if isinstance(target, CallbackQuery):
        try:
            await target.message.edit_text(text, parse_mode="HTML", reply_markup=_mode_kb())
        except Exception:
            await target.message.answer(text, parse_mode="HTML", reply_markup=_mode_kb())
        await target.answer()
    else:
        await target.answer(text, parse_mode="HTML", reply_markup=_mode_kb())


@router.message(Command("ai"))
async def cmd_ai(message: Message, state: FSMContext) -> None:
    await _show_ai_menu(message, state)


@router.callback_query(F.data == "ai_assist")
async def cb_ai_assist(cb: CallbackQuery, state: FSMContext) -> None:
    await _show_ai_menu(cb, state)
...
async def process_ai_text(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        # Merge PDF button + menu button
        if pdf_kb:
            kb = InlineKeyboardMarkup(inline_keyboard=[
                *pdf_kb.inline_keyboard,
                [InlineKeyboardButton(text="◀️ Меню", callback_data="menu")],
            ])
        else:
            kb = _menu_kb()

        await message.reply(reply, parse_mode="HTML", reply_markup=kb)
        ...
        await thinking.delete()
        await message.answer("❌ Ошибка AI. Попробуй позже.", reply_markup=_menu_kb())
...
async def _price_from_text(text: str, user_id: int) -> dict:
    ...
    if m:
        ...
        from src.services.ai_kimi import kimi_service as _ks
        parsed = await _ks.logist_mode(text)
    ...
    from src.services.ai_kimi import kimi_service as _ks2
    return await _ks2.price_mode(
        from_city=from_city,
        to_city=to_city,
        weight=weight,
        vehicle=vehicle,
