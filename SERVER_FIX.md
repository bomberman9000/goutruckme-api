...
docker compose exec -T db psql -U postgres -d gruzpotok -c '\dt'
...
# 4) Проверить
docker compose exec -T db psql -U postgres -d gruzpotok -c '\dt'
docker compose exec -T db psql -U postgres -d gruzpotok -c 'select version_num from alembic_version;'
